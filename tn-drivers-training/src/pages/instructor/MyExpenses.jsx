      ...
      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
        {filteredExpenses.map((exp) => (
          <tr key={exp.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors group">
            ...
            <td className="px-6 py-4">
              <span className="text-base font-bold text-teal-600 dark:text-teal-400">${exp.amount.toFixed(2)}</span>
              <p className="text-[10px] text-slate-400 mt-0.5">{exp.payment_method}</p>
             </td>
            <td className="px-6 py-4">
              <span className="text-sm font-medium text-slate-800 dark:text-slate-400">{exp.created_at}</span>
             </td>
          ...
          <div className="bg-white dark:bg-slate-950 w-full max-w-md rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <DetailItem label="Amount" value={`$${selectedExpense.amount.toFixed(2)}`} />
                ...
                <DetailItem label="Date" value={selectedExpense.created_at} />
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-950 w-full max-w-md rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
              ...
              <button onClick={() => { setIsModalOpen(false); setIsEditingExpense(false); }} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg">
                <X size={18} />
              </button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <ExpenseSelect label="Payment Method" options={['cash', 'online', 'card']} value={formData.payment_method} onChange={(v)=>setFormData({...formData, payment_method:v})}/>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea 
                  className="w-full px-4 py-2.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-sm outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all resize-none" 
                  rows="3"
