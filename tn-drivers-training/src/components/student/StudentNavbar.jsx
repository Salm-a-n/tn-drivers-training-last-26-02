...
import { Bell, Home, Clock } from "lucide-react";
...
const StudentNavbar = ({ isOpen, setIsOpen }) => {
  ...
  // Simulate unread count for student (you can replace with API call)
  useEffect(() => {
  ...
  // Get page title without "student/" prefix
  const getPageTitle = () => {
      ...
      <header className="relative flex items-center justify-between px-4 md:px-6 py-2.5 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {/* <button 
            ...
            title="Notifications"
          >
            <Bell className="w-4 h-4 sm:w-5 sm:h-5 text-slate-600 dark:text-slate-400 group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors" strokeWidth={1.8} />
            {unreadCount > 0 && (
            ...
            {user?.profile_picture ? (
              <img
                ...
                onError={(e) => {
                  ...
                  e.target.src = `https://ui-avatars.com/api/?name=${user?.name || 'Student'}&background=008B8B&color=fff&rounded=true&size=32`;
                }}
              />
            ) : (
